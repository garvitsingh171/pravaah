...
| Version                 | `v0.2.0`                                       |
| Status                  | Active development release                     |
| Previous stable release | `v0.1.0` - MVP complete, deployed, and frozen  |
| Source basis            | `docs/PRAVAAH_V0_2_SCOPE_AND_TRANSITION.md`    |

`v0.2.0` turns the completed MVP from an owner-provisioned demo into a safe self-service product demonstration. It must preserve the existing clinic workflow while adding public entry, Clerk sign-up, transactional clinic bootstrap, isolated sample data, onboarding-aware routing, settings, tests, and release documentation.
...
- Complete clinic settings, doctor edit, patient edit, and queue reorder UI gaps.
...
Until then, v0.2 remains active development and its product features must not be described as already implemented.
