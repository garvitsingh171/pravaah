...
| `/dashboard`         | `DashboardOverviewPage` | Protected Admin/Staff      | Protected app shell         | Dashboard summary, high-risk appointments, checklist status               | Loading, empty-ish metric states, error/retry                         | Responsive metric grids and constrained page content                                                              | Lazy protected route chunk                                             | Page heading, readable status copy, risk explanations                    |
...
| `/queue`             | `QueuePage`             | Protected Admin/Staff      | Protected app shell         | Today's queue, doctors, status/reorder APIs, risk details                 | Loading, empty, filtered empty, API error/retry, reorder/status toast | Filter controls stack; active queue table uses labeled, keyboard-focusable horizontal scrolling on small screens  | Lazy protected route chunk; queue reorder stays user-triggered         | Move/status buttons are native controls; risk unavailable is explicit    |
...
| Dense tables          | Doctor, patient, appointment, and active queue tables now expose their horizontal scroll areas to keyboard users with route-specific labels.                                                                   | `apps/web/src/features/doctors/DoctorsPage.tsx`, `apps/web/src/features/patients/PatientsPage.tsx`, `apps/web/src/features/appointments/AppointmentsPage.tsx`, `apps/web/src/features/queues/QueuePage.tsx` |
...
| Dense tables can be horizontally scrolled by touch and keyboard focus                                    | `/doctors`, `/patients`, `/appointments`, `/queue`                                         |
...
5. Keyboard-scroll the doctors, patients, appointments, and active queue table regions.
6. Submit invalid forms and confirm focus remains usable and field errors are visible.
7. Trigger API error states with the backend stopped and confirm retry buttons are reachable.
8. Confirm risk unavailable text is not presented as low risk.
