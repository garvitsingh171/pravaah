...
- production monitoring and deployment runbooks
- audit logging
- notification/reminder provider
- whether to generate OpenAPI from Zod schemas
- when enough data exists for trained ML

## Why Manual Queue Reorder Controls

v0.2 exposes human-controlled queue movement in the frontend instead of automatic queue optimization. Staff can adjust order when clinic reality changes, while the backend still validates that the reorder request includes exactly the active queue entries for the selected date and clinic.

## Why Render Build Uses Production Tsconfig

The backend build uses `tsconfig.build.json` so deployed output excludes tests and test helpers. That keeps Render-style production builds focused on runtime code while allowing Vitest and TypeScript-aware tests to keep their own test-inclusive configuration.
