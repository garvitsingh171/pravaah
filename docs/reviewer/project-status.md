...
| Clinic provisioning      | `IMPLEMENTED`           | `DEPLOYED`        | `auth.repository.ts`, `auth.service.ts`, owner-reported clinic provisioning PASS                                                            | Creates first Admin; v0.4 source also blocks identities with pending Staff invitations.                           |
| Staff invitations/access | `IMPLEMENTED`           | `NOT_VERIFIED`    | `staff` server/web features, `StaffInvitation` schema, focused tests                                                                        | Copy-link workflow only; migration/generation/test/build and deployment verification remain owner tasks.          |
...
| Clinic settings          | `IMPLEMENTED`           | `DEPLOYED`        | `ClinicSettingsPage.tsx`, `clinic.routes.ts`, owner-reported Admin flow PASS                                                                | Admin-only; no active-state settings edit endpoint.                                                               |
...
| Patient management       | `IMPLEMENTED`           | `DEPLOYED`        | `apps/web/src/features/patients`, `apps/server/src/modules/patients`, structured location validation/repository tests                         | Patient history counters exist but are not automatically maintained by lifecycle updates; #263 location migration remains an owner-applied create-only migration. |
| Appointment management   | `IMPLEMENTED`           | `DEPLOYED`        | `AppointmentsPage.tsx`, `appointment.service.ts`, owner-reported appointment flow PASS                                                      | Slot discovery enforces weekly availability, hours, duration, and buffer; no rescheduling endpoint yet.           |
