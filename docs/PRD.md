...
| Appointment lifecycle     | Manual status updates, queue sync, and first-arrival late classification. | Implemented but not yet released | Admin, Staff            | `appointment.lifecycle.ts`, `appointment.arrival.ts`, `appointment.repository.ts`                | Strict transition graph is server-enforced; no full attendance analytics.                       | [Roadmap](scope/ROADMAP.md)                                                         |
...
| No-show risk              | Deterministic stored risk score and reasons.      | Implemented but not yet released | Admin, Staff            | `prediction.service.ts`                                                                        | Not trained ML; non-late attendance counters can drift.                                         | [HLD](HLD.md)                                                                       |
...
| `COMPLETED` | Visit completed.             | `IN_QUEUE`, `CALLED`, or same-status retry.                  | Same-status retry only.                                    | Queue status becomes `COMPLETED` and `completedAt` may be set. | Does not establish arrival by itself.           | Yes   | No.                 |
...
| `NO_SHOW`   | Patient did not attend.      | `SCHEDULED`, `CONFIRMED`, `ARRIVED`, `IN_QUEUE`, `CALLED`, or same-status retry. | Same-status retry only.                         | Queue status becomes `NO_SHOW`.                                | No implemented counter update on status change. | Yes   | No.                 |
...
- Remaining risks include duration-overlap scheduling, patient/clinic counter drift, and release verification evidence.
...
| Counter drift.                                      | Appointment counts used directly in scoring for no-show/completed; late arrivals now increment from recorded first-arrival facts. | Some non-late PatientClinic counters are not auto-updated. |
