...
Current restrictions: Admin can invite, suspend, and reactivate Staff in the same clinic, but cannot edit roles, transfer Admin ownership, delete users, or manage multi-clinic membership. There is no patient/doctor portal administration.
...
Current restrictions: Staff cannot access Admin-only clinic settings or Staff-management APIs and do not see those navigation items. Staff cannot invite users, change roles, create a clinic from a pending invitation, or reactivate themselves.
...
| Appointment cancellation/no-show   | Yes                         | Yes                         | No                                            | No                                            | Implemented but not yet released | Requires a bounded structured reason, accepts an optional note, and syncs queue where present. |
...
| Staff invitations         | Admin creates/revokes seven-day invitations; matching Clerk identity accepts into clinic.  | Implemented but not yet released | Admin, invited Staff    | `staff` server/web features, `StaffInvitation`                                                 | Copy-link delivery only; no mail provider.                                                                  | [Staff workflow](workflows/staff-invitations-and-management.md)                     |
| Staff access management   | Admin lists team and suspends/reactivates Staff without deleting historical identity.      | Implemented but not yet released | Admin                   | `/staff`, Staff management APIs                                                                | No role editor, Admin transfer, hard delete, or multi-clinic membership.                                    | [User Roles](product/USER_ROLES.md)                                                 |
...
| Clinic settings           | Admin clinic profile and operating settings, including late-arrival grace.                 | Implemented but not yet released | Admin                   | `clinics` module, `ClinicSettingsPage`                                                         | Slug and active state are read-only in current API.                                                         | [API Reference](architecture/API_REFERENCE.md)                                      |
...
| Doctor management         | Create/list/edit doctor records and recurring weekly availability.                         | Implemented but not yet released | Admin, Staff            | `doctors` module and feature                                                                   | No detail route; link status update not exposed; no date-specific exception/leave model.                    | [User Roles](product/USER_ROLES.md)                                                 |
| Patient management        | Create/list/edit patient records and view event-maintained clinic history.                 | Implemented but not yet released | Admin, Staff            | `patients` module and feature                                                                  | Operational statistics are read-only; link status filtering has a known gap when link is inactive.          | [Roadmap](scope/ROADMAP.md)                                                         |
| Appointment booking       | Discovers available slots, creates appointment, queue entry, prediction.                   | Implemented but not yet released | Admin, Staff            | `appointments` module                                                                          | No past-date business rejection.                                                                            | [API Reference](architecture/API_REFERENCE.md)                                      |
...
| Appointment rescheduling  | Moves an eligible existing appointment to another valid slot.                              | Implemented but not yet released | Admin, Staff            | `appointments` module                                                                          | No doctor reassignment, patient reassignment, duration editing, notifications, or prediction recalculation. | [API Reference](architecture/API_REFERENCE.md)                                      |
...
| Appointment lifecycle     | Manual status updates, queue sync, arrival classification, and patient outcome aggregates. | Implemented but not yet released | Admin, Staff            | appointment lifecycle/arrival/statistics repositories                                          | Strict transition graph and first-transition idempotency are server-enforced; no analytics dashboard.       | [Roadmap](scope/ROADMAP.md)                                                         |
| Queue management          | List, status update, queue timestamps, and synchronized arrival display.                   | Implemented but not yet released | Admin, Staff            | `queues` module                                                                                | No standalone arrival endpoint; queue entry is created during booking.                                      | [HLD](HLD.md)                                                                       |
...
| No-show risk              | Deterministic stored risk score and reasons.                                               | Implemented but not yet released | Admin, Staff            | `prediction.service.ts`                                                                        | Not trained ML; attendance maintenance does not recalculate stored predictions.                             | [HLD](HLD.md)                                                                       |
...
Appointment slot discovery requires clinic access, an active clinic, active linked doctor, clinic-local date, and duration. It returns slots generated from clinic hours, slot duration, buffer, doctor weekly availability, and existing active appointments. Appointment creation requires a generated `scheduledAt`, active linked patient, reason/notes when provided, booking source, and creator. It revalidates the selected slot, rejects active duration-plus-buffer overlaps, and creates an appointment, queue entry, prediction, and booking-count increment in one transaction. Past-date rejection remains a business-rule gap.

Appointment rescheduling is staff-operated and appointment-specific. It is allowed only for `SCHEDULED` and `CONFIRMED` appointments, keeps the same appointment row, preserves lifecycle status, derives doctor/patient/duration from persisted data, reuses the canonical slot engine with current-appointment self-exclusion, and updates `Appointment.scheduledAt` in a guarded transaction. The existing `QueueEntry` is preserved; same-day moves keep position, while cross-date moves assign the next destination doctor/date position. No notification, doctor reassignment, patient reassignment, duration editing, history model, or prediction recalculation is included. Status updates are manual, lifecycle-guarded, and synchronized to queue where applicable. Status: Implemented but not yet released.
...
| ----------- | ---------------------------- | -------------------------------------------------------------------------------- | ---------------------------------------------------------- | -------------------------------------------------------------- | ----------------------------------------------------------------------------------- | ----- | ----------------------- |
| `SCHEDULED` | Booked appointment.          | Creation or same-status retry.                                                   | `CONFIRMED`, `ARRIVED`, `IN_QUEUE`, `CANCELLED`, `NO_SHOW` | No queue status change when set explicitly.                    | None.                                                                               | No    | No backward transition. |
| `CONFIRMED` | Staff-confirmed appointment. | `SCHEDULED` or same-status retry.                                                | `ARRIVED`, `IN_QUEUE`, `CANCELLED`, `NO_SHOW`              | No queue status change.                                        | None.                                                                               | No    | No backward transition. |
| `ARRIVED`   | Patient has arrived.         | `SCHEDULED`, `CONFIRMED`, or same-status retry.                                  | `IN_QUEUE`, `CALLED`, `CANCELLED`, `NO_SHOW`               | Queue status becomes `ARRIVED`.                                | First arrival snapshot; late count increments only if late.                         | No    | No backward transition. |
| `IN_QUEUE`  | Patient is waiting in queue. | `SCHEDULED`, `CONFIRMED`, `ARRIVED`, or same-status retry.                       | `CALLED`, `COMPLETED`, `CANCELLED`, `NO_SHOW`              | Queue status becomes `WAITING`.                                | First arrival snapshot; late count increments only if late.                         | No    | No backward transition. |
| `CALLED`    | Patient has been called.     | `ARRIVED`, `IN_QUEUE`, or same-status retry.                                     | `COMPLETED`, `CANCELLED`, `NO_SHOW`                        | Queue status becomes `CALLED` and `calledAt` may be set.       | First arrival snapshot if not already recorded.                                     | No    | No backward transition. |
| `COMPLETED` | Visit completed.             | `IN_QUEUE`, `CALLED`, or same-status retry.                                      | Same-status retry only.                                    | Queue status becomes `COMPLETED` and `completedAt` may be set. | First transition increments completed visits and advances last visit monotonically. | Yes   | No.                     |
| `CANCELLED` | Appointment cancelled.       | `SCHEDULED`, `CONFIRMED`, `ARRIVED`, `IN_QUEUE`, `CALLED`, or same-status retry. | Same-status retry only.                                    | Queue status becomes `CANCELLED`.                              | None.                                                                               | Yes   | No.                     |
| `NO_SHOW`   | Patient did not attend.      | `SCHEDULED`, `CONFIRMED`, `ARRIVED`, `IN_QUEUE`, `CALLED`, or same-status retry. | Same-status retry only.                                    | Queue status becomes `NO_SHOW`.                                | First transition increments no-shows.                                               | Yes   | No.                     |

Service authority: `appointment.service.ts`, `appointment.lifecycle.ts`, and `appointment.repository.ts`.

### Queue Management

Queue entries are created during appointment booking. Queue list is scoped by clinic and appointment clinic-local date. Position assignment is scoped by clinic, doctor, and clinic-local date. Active statuses are `ARRIVED`, `WAITING`, and `CALLED`; final statuses are `COMPLETED`, `CANCELLED`, and `NO_SHOW`. Queue status updates synchronize appointment status after passing the server-enforced queue lifecycle: `WAITING` can move to `ARRIVED`, `CALLED`, `COMPLETED`, `CANCELLED`, or `NO_SHOW`; `ARRIVED` can move to `WAITING`, `CALLED`, `CANCELLED`, or `NO_SHOW`; `CALLED` can move to `COMPLETED`, `CANCELLED`, or `NO_SHOW`; final statuses are terminal. Reordering excludes final entries, requires the complete active queue set for one doctor and clinic-local date, uses a clinic/doctor/date advisory lock, and uses a two-phase position update transaction. Status: Implemented but not yet released; owner test and browser verification remain pending.
...
flowchart TD
    ...
    C -- No --> E{Queue transition allowed?}
    E -- No --> I[Lifecycle conflict error]
    E -- Yes --> F[Transaction updates QueueEntry]
    F --> G[Synchronize Appointment status]
    G --> J[Set calledAt or completedAt when relevant]
    J --> H[Return updated queue entry]
```
...
| PR-STAFF-001  | Staff invitation        | Only same-clinic active Admin can create a normalized, expiring, one-time Staff invitation.      | Admin         | Valid email; no member/pending invite conflict.   | Hash persists; raw URL returned once; no User created yet.                   | Admin and clinic guards.                       | Implemented but not yet released | `staff.routes.ts`, `staff.service.ts`                 | No automatic email delivery.                         |
| PR-STAFF-002  | Invitation acceptance   | Valid token and matching trusted Clerk email atomically create/activate STAFF/ACTIVE membership. | Invited Staff | Pending, unexpired invitation.                    | User and accepted invitation commit together or roll back.                   | Clerk identity plus invitation proof.          | Implemented but not yet released | `staff.repository.ts`                                 | Suspended/Admin/other-clinic conflicts are rejected. |
| PR-STAFF-003  | Onboarding protection   | Pending Staff invitee must not create a clinic and become Admin.                                 | Invited Staff | Trusted email has unexpired pending invitation.   | `STAFF_INVITATION_PENDING`; no clinic or Admin is created.                   | Shared normalized-email advisory lock.         | Implemented but not yet released | `auth.service.ts`, `auth.repository.ts`               | Token still required for acceptance.                 |
| PR-STAFF-004  | Staff access management | Admin may suspend/reactivate same-clinic Staff without deletion or role editing.                 | Admin         | Target is Staff in expected source status.        | Only `ACTIVE <-> SUSPENDED`; historical actors remain.                       | Admin and clinic guards.                       | Implemented but not yet released | `staff` module                                        | Existing sessions fail next protected API request.   |
...
| PR-APT-002    | Appointment conflict    | Active doctor appointment overlaps must be rejected using duration plus clinic buffer.           | Admin, Staff  | Same clinic/doctor effective scheduling interval. | Returns conflict.                                                            | Admin/Staff clinic access.                     | Implemented but not yet released | `appointment.service.ts`, `appointment.repository.ts` | Overlapping duration/buffer slots fail.              |
...
| PR-APT-006    | Appointment reschedule  | Eligible appointments can move to another valid slot without replacement.                        | Admin, Staff  | `SCHEDULED` or `CONFIRMED` appointment.           | Same appointment ID, status, queue entry, and prediction relation preserved. | Appointment clinic access.                     | Implemented but not yet released | `appointment.service.ts`, `appointment.repository.ts` | No doctor/patient/duration edit or notifications.    |
...
- Patient aggregates are updated in the same transactions as booking and first terminal lifecycle transitions; no reconciliation tool is included.
...
- Appointment and queue terminal outcomes use an exact expected appointment-status compare-and-set, so retries and competing terminal transitions cannot both own aggregate effects.
- Onboarding relies on unique constraints and post-conflict current-identity re-read.
- Remaining risks include aggregate reconciliation for legacy/manual anomalies and release verification evidence.
...
| BR-008 | Appointment duration-plus-buffer conflicts must be rejected for active doctor appointments.                         | Implemented but not yet released | appointment scheduling policy    |
...
| Appointment conflicts beyond exact same start time. | Scheduling policy and day-scoped advisory lock reject duration-plus-buffer overlaps.                                                 | Broader concurrency stress testing remains future work.       |
...
| Aggregate drift.                                    | Booking, first completion, first no-show, and first late-arrival effects update `PatientClinic` in their authoritative transactions. | No historical reconciliation/admin correction workflow.       |
| Risk-score overclaiming.                            | Docs and UI should call it deterministic/advisory.                                                                                   | Must avoid marketing drift.                                   |
| Observability gaps.                                 | Health endpoint, basic logging, and appointment operational activity.                                                                | No monitoring, tracing, global security audit log, or alerts. |
...
- What broader security/compliance audit logging is required beyond the appointment operational timeline?
...
| PR-APT-001     | Appointment booking           | Implemented but not yet released | `/appointments`                    | appointments module                               | `Appointment`, `QueueEntry`, `NoShowPrediction` | appointment tests                   | API Reference                | No past-date business rejection            |
| PR-APT-004     | Appointment lifecycle         | Implemented but not yet released | `/appointments`                    | status endpoint                                   | `Appointment`, `QueueEntry`                     | appointment status tests            | Audit                        | Route-level lifecycle coverage can expand  |
| PR-APT-006     | Appointment activity timeline | Implemented but not yet released | `/appointments` activity dialog    | `GET /api/appointments/:appointmentId/activities` | `AppointmentActivity`                           | activity/status/queue tests         | API and database design      | Legacy history is intentionally incomplete |
...
### Appointment operational timeline boundary
