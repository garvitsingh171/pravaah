...
| Queue reordering                   | Yes                         | Yes                         | No                                            | No                                            | Implemented but not yet released | Active clinic/date queue IDs must be complete; known doctor-scope limitation.                  |
...
| Protected routes          | App shell and route guards.                       | Implemented but not yet released | Admin, Staff            | `ProtectedAppShell`, `dashboardRoutes.tsx`                                                     | Direct runtime verification pending.                                   | [HLD](HIGH_LEVEL_DESIGN.md)                        |
...
| Clinic provisioning       | Transactional clinic plus first Admin creation.   | Implemented but not yet released | New Admin               | `auth.repository.ts`, `auth.service.ts`                                                        | Live rollback verification pending.                                    | [HLD](HIGH_LEVEL_DESIGN.md)                        |
...
| Appointment detail        | Details shown inside list response/panels.        | Implemented but not yet released | Admin, Staff            | `AppointmentsPage`, list response                                                              | No dedicated detail endpoint or route.                                 | [HLD](HIGH_LEVEL_DESIGN.md)                        |
...
| Queue management          | List, status update, queue timestamps.            | Implemented but not yet released | Admin, Staff            | `queues` module                                                                                | No standalone arrival endpoint; queue entry is created during booking. | [HLD](HIGH_LEVEL_DESIGN.md)                        |
| Queue reordering          | Manual active queue reorder.                      | Under development                | Admin, Staff            | `queue.service.ts`, `QueuePage`                                                                | Backend validates clinic/date active set, not doctor-scoped set.       | [Audit](audits/V0.3_ROUTE_RELEASE_AUDIT.md)        |
| No-show risk              | Deterministic stored risk score and reasons.      | Implemented but not yet released | Admin, Staff            | `prediction.service.ts`                                                                        | Not trained ML; counters can drift.                                    | [HLD](HIGH_LEVEL_DESIGN.md)                        |
| Public fallback pages     | Not-found and guard fallback states.              | Implemented but not yet released | All                     | `NotFoundPage`, error boundaries, route guards                                                 | No dedicated unauthorized route.                                       | [HLD](HIGH_LEVEL_DESIGN.md)                        |
...
| Advanced machine learning | Trained prediction model.                         | Explicitly out of scope          | Future                  | Current deterministic service                                                                  | Requires dataset and validation.                                       | [HLD](HIGH_LEVEL_DESIGN.md)                        |
...
| PR-QUEUE-003  | Queue reorder           | Active queue reorder must reject missing or final entries.                                      | Admin, Staff  | Reorder request.                            | Valid reorder sets positions; invalid set fails.                   | Admin/Staff clinic access.                     | Under development                | `queue.service.ts`, `queue.repository.ts`            | Doctor-scope gap remains.                           |
...
| BR-011 | Queue reorder must exclude final entries and use all active entries for the scoped date. | Under development                | queue service; doctor-scope gap  |
...
| PR-QUEUE-003   | Queue reorder               | Under development                | `/queue`                           | reorder endpoint                       | `QueueEntry`                                    | queue reorder tests                 | Audit                 | Doctor-scope validation gap                  |
