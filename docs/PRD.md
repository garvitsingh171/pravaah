...
Pravaah provides an internal Admin/Staff timeline for meaningful committed appointment creation, first-arrival, lifecycle, and reschedule events. It records the authenticated actor where available and structured event facts. It is not a full compliance/security audit system, a patient-facing timeline, analytics, reason capture, manual notes, notification history, or event-sourced replacement for current appointment state.
