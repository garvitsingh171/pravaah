...
| Appointment lifecycle     | Manual status updates and queue sync.             | Implemented but not yet released | Admin, Staff            | `appointment.lifecycle.ts`, `appointment.repository.ts`                                         | Strict transition graph is server-enforced.                                                     | [Roadmap](scope/ROADMAP.md)                                                         |
...
| `ARRIVED`   | Patient has arrived.         | `SCHEDULED`, `CONFIRMED`, or same-status retry.              | `IN_QUEUE`, `CALLED`, `CANCELLED`, `NO_SHOW`               | Queue status becomes `ARRIVED`.                                | None.                                           | No    | No backward transition. |
| `IN_QUEUE`  | Patient is waiting in queue. | `SCHEDULED`, `CONFIRMED`, `ARRIVED`, or same-status retry.   | `CALLED`, `COMPLETED`, `CANCELLED`, `NO_SHOW`              | Queue status becomes `WAITING`.                                | None.                                           | No    | No backward transition. |
| `CALLED`    | Patient has been called.     | `ARRIVED`, `IN_QUEUE`, or same-status retry.                 | `COMPLETED`, `CANCELLED`, `NO_SHOW`                        | Queue status becomes `CALLED` and `calledAt` may be set.       | None.                                           | No    | No backward transition. |
| `COMPLETED` | Visit completed.             | `IN_QUEUE`, `CALLED`, or same-status retry.                  | Same-status retry only.                                    | Queue status becomes `COMPLETED` and `completedAt` may be set. | No implemented counter update on status change. | Yes   | No.                 |
