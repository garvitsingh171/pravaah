...
| 3 | Enter date/time, duration, source, notes | Frontend collects fields; backend is authority. | Submit enabled if local checks pass. | `AppointmentBookingForm.tsx -> handleSubmit` | Does frontend validation replace Zod? | Say no; show backend schema. |
| 4 | Submit | The frontend calls typed API helper. | Network request to `/api/clinics/:clinicId/appointments`. | `appointmentApi.ts -> createAppointment` | How is clinic ID encoded? | Show `encodeURIComponent`. |
