...
This database summary is derived from `apps/server/prisma/schema.prisma` and should stay aligned with [High-Level Design](../HLD.md), [backend/database LLD section](../LLD.md#backend-database-and-workflow-implementation), and the [Workflow Atlas](../workflows/README.md). The HLD explains system structure, the LLD explains implementation details, the atlas traces database use per workflow, and this file remains the database-focused reference.
