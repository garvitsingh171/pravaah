...
For exact end-to-end code traces, use the [Workflow Atlas](workflows/README.md). This HLD owns system architecture and major boundaries; the atlas owns workflow evidence from frontend handlers through middleware, services, repositories, Prisma, database constraints, responses, and final UI state.
...
is not travel time. #263 provides address storage and geocoding; #267 adds
backend-only Patient-to-Clinic road routing from current trusted coordinates.
The route is stored compactly on `PatientClinic`, with no map geometry or live
traffic claim.
...
| Doctor availability | `DoctorAvailabilityPeriod` rows for one `DoctorClinic`           | Full week must be valid before replacement.                             | Delete old periods, insert replacement periods, re-read.                   | Rollback.                                 | `doctor.repository.ts`      | No date-specific exceptions.                                    |
| Patient creation    | `Patient`, `PatientClinic`                                       | Patient history must be clinic-linked.                                  | Both records with zero aggregate defaults.                                 | Rollback.                                 | `patient.repository.ts`     | Link active field not exposed.                                  |
...
| Appointment booking | `Appointment`, `QueueEntry`, `NoShowPrediction`, `PatientClinic` | Booking creates operational queue/risk context and increments bookings. | All related writes.                                                        | Rollback; conflicts mapped.               | `appointment.repository.ts` | No past-date business rejection.                                |
| Appointment status  | `Appointment`, mapped `QueueEntry`, `PatientClinic`              | Keep status and clinic-specific outcomes consistent.                    | Exact transition, queue sync, first terminal outcome.                      | Conflict or rollback.                     | `appointment.repository.ts` | Transition policy is centralized in `appointment.lifecycle.ts`. |
| Queue status        | `QueueEntry`, `Appointment`, `PatientClinic`                     | Keep queue, appointment, and terminal outcomes synchronized.            | Exact appointment transition, queue status, timestamps, outcome aggregate. | Conflict or rollback.                     | `queue.repository.ts`       | Reuses the appointment outcome policy.                          |
...
| Appointment conflict | Two bookings with overlapping doctor time.                                         | PostgreSQL advisory lock by doctor clinic-local day plus duration/buffer overlap query.                                       | Clinic, doctor, clinic-local date.            | Broader concurrency stress testing remains future work.  |
...
| Appointment outcome  | Retry, competing terminal status, or appointment/queue race double-counts history. | Exact expected appointment-status compare-and-set; only its winner invokes shared patient statistics in the same transaction. | Appointment and clinic-specific patient link. | Manual retry/refresh may be needed after true conflicts. |
| Queue status         | Concurrent final/status updates.                                                   | Appointment-first lock order, exact queue/appointment guards, and same-target retry classification.                           | Queue entry and appointment.                  | Manual retry/refresh may be needed after conflicts.      |
...
Appointment creation atomically increments `PatientClinic.totalAppointments`. Appointment and queue status paths share one outcome policy: the exact first transition to `COMPLETED` increments `totalCompletedVisits` and conditionally advances `lastVisitAt`; the exact first transition to `NO_SHOW` increments `totalNoShows`. `lastVisitAt` uses the same completion event timestamp as `QueueEntry.completedAt` and is updated only when null or older. Prediction feature sources and rule math are unchanged.
...
| Appointment conflict        | `APPOINTMENT_SLOT_CONFLICT`.                                                           | Staff may need to refresh if another booking takes the slot first. |
...
- Doctor recurring weekly availability powers appointment slot discovery and final booking validation.
...
React never receives the provider key or calls Geoapify. After both sides are
geocoded, the backend may call Geoapify Routing with Patient → Clinic
coordinates, outside any Prisma transaction, and conditionally persist:

```text
Patient coordinates + Clinic coordinates
    → Geoapify Routing (drive, balanced, free_flow)
    → distance meters / 1000, time seconds rounded up to minutes
    → PatientClinic distanceFromClinicKm + estimatedTravelTimeMinutes
```

Routing failure never rolls back Patient/Clinic core writes. Clinic coordinate
changes invalidate related routes without mass provider calls; Patient changes
invalidate all related routes and recalculate only the current Clinic link.
