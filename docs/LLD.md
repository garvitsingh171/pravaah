...
| `/sign-up/*`         | Authentication                  | Public/auth                                  | `AuthPageLayout`             | `SignUpPage`            | Clerk sign-up.                                                                        | Safe internal `redirect_url`, otherwise `/onboarding/clinic`.               | `noindex,nofollow`.            |
...
| `/invite/:token`     | Public-shell Staff invitation   | Clerk identity; internal user may be missing | Standalone invitation layout | `StaffInvitationPage`   | Authenticate, preview, and deliberately accept a Staff invitation.                    | Login/signup preserves safe internal return path; success opens dashboard.  | `noindex,nofollow`.            |
...
| `/appointments`      | Protected application           | Admin/Staff                                  | `AppLayout`                  | `AppointmentsPage`      | Book/filter/status/reschedule appointments, show risk, and open operational activity. | Guarded by shell.                                                           | `noindex,nofollow`.            |
...
| `/staff`             | Admin-only protected app        | Admin backend authority; Staff route blocked | `AppLayout`                  | `StaffManagementPage`   | Team list, invite history, suspend/reactivate, revoke/reinvite, one-time copy link.   | Frontend route metadata plus authoritative Admin-only backend APIs.         | `noindex,nofollow`.            |
...
- Staff management is implemented through the focused `features/staff` frontend and `modules/staff` backend layers.
...
| GET    | `/api/staff/invitations/:token`                                 | Clerk identity       | token params                       | `previewStaffInvitationController`         | `staffService.previewInvitation`             | No                       | minimal invitation preview           | invalid/mismatch/status.                          |
| POST   | `/api/staff/invitations/:token/accept`                          | Clerk identity       | token params + strict empty body   | `acceptStaffInvitationController`          | `staffService.acceptInvitation`              | Yes                      | Staff user/clinic/outcome            | expired/revoked/identity/member conflicts.        |
| GET    | `/api/clinics/:clinicId/staff`                                  | Admin                | clinic params                      | `listStaffController`                      | `staffService.listMembers`                   | No                       | Admin/Staff member summaries         | access/not found.                                 |
| PATCH  | `/api/clinics/:clinicId/staff/:userId/status`                   | Admin                | params + ACTIVE/SUSPENDED body     | `updateStaffStatusController`              | `staffService.updateStaffStatus`             | Yes                      | updated Staff summary                | target/transition/access conflicts.               |
| GET    | `/api/clinics/:clinicId/staff/invitations`                      | Admin                | clinic params                      | `listStaffInvitationsController`           | `staffService.listInvitations`               | No                       | invitation history/effective status  | access/not found.                                 |
| POST   | `/api/clinics/:clinicId/staff/invitations`                      | Admin                | clinic params + strict email body  | `createStaffInvitationController`          | `staffService.createInvitation`              | Yes                      | invitation + one-time URL            | duplicate/member/access conflicts.                |
| PATCH  | `/api/clinics/:clinicId/staff/invitations/:invitationId/revoke` | Admin                | clinic/invitation params           | `revokeStaffInvitationController`          | `staffService.revokeInvitation`              | Yes                      | revoked invitation                   | expired/accepted/replay/access conflicts.         |
...
No standalone prediction, patient-auth, doctor-auth, notification,
...
-> atomically increment PatientClinic.totalAppointments
```
...
| ----------------- | --------------------------- | ------------------------------- | ----------------------------- | -------------------------------- | ----------- | ----------------------------------------------------------------------------------- |
| Book appointment  | valid clinic/doctor/patient | `SCHEDULED` appointment created | `WAITING` queue entry created | `queuedAt` default               | Yes         | Increment total appointments.                                                       |
...
| Complete          | non-final                   | `COMPLETED`                     | `COMPLETED`                   | `completedAt` if empty           | Yes         | First transition increments completed visits and advances last visit monotonically. |
| Cancel            | non-final                   | `CANCELLED`                     | `CANCELLED`                   | updatedAt                        | Yes         | No arrival or counter increment.                                                    |
| No-show           | non-final                   | `NO_SHOW`                       | `NO_SHOW`                     | updatedAt                        | Yes         | First transition increments no-shows; last visit is unchanged.                      |
...
| Appointment booking           | `Appointment`, `QueueEntry`, `NoShowPrediction`, `PatientClinic` | Advisory locks plus atomic booking-count increment.                                         | No past-date business rejection.                   |
...
### v0.4 Staff Invitation And Management Detail

- Token creation uses `randomBytes(32).toString('base64url')`; lookup uses `createHash('sha256')`. Only `tokenHash` is stored, and the raw URL is returned once from creation.
- `normalizeEmail` is exactly `email.trim().toLowerCase()` and is reused by Clerk resolution, creation, identity matching, and owner-onboarding protection.
- Admin management routes use `authenticateRequest -> requireAdminRole -> strict validation -> requireClinicAccess`. Preview and acceptance use `authenticateClerkIdentity` because an internal user may not exist.
- Invitation creation takes normalized-email and clinic/email advisory transaction locks before rechecking existing users and active pending invitations.
- Acceptance resolves Clerk outside the transaction, locks the invitation row, enforces `PENDING` and `expiresAt > now`, checks identity conflicts, creates/activates `STAFF`/`ACTIVE`, and conditionally claims the invitation with one event timestamp.
- Revocation locks the same row and also claims only unexpired `PENDING`, so accept/revoke cannot both commit. Same-user double acceptance is an idempotent `ALREADY_ACCEPTED` result.
- Expiry is a response/service projection: persisted `PENDING` plus `expiresAt <= now` becomes effective `EXPIRED`; no scheduler is needed.
- Owner provisioning uses the same normalized-email lock and performs an in-transaction invitation recheck. `STAFF_INVITATION_PENDING` prevents clinic/Admin creation but does not silently accept membership without the token.
- `/invite/:token` is public-shell; `/staff` is protected and Admin-only. Login and signup share internal-only redirect validation, preserving the invite path without allowing open redirects.
- Status management permits only `ACTIVE <-> SUSPENDED` for `STAFF`. No user deletion or appointment-activity rewrite occurs.

### Backend Limitations

- Backend lint script is a placeholder.
- No explicit 404 fallback middleware is registered.
- Staff management intentionally excludes role editing, Admin transfer, hard deletion, multi-clinic membership, and automatic email delivery.
...
`AppointmentActivityType` and `AppointmentActivity` provide an append-only secondary history; `Appointment` and `QueueEntry` remain current-state truth. `appointment.activity.ts` owns the single status-to-event map, structured metadata builders, JSON-safe ISO conversion, and same-time logical ordering. `appointment.activity.repository.ts` accepts an existing `Prisma.TransactionClient`, inserts activities, and performs the clinic-scoped read with minimal actor projection.

Booking inserts `APPOINTMENT_CREATED` after appointment/queue/prediction/statistic writes but before transaction commit, using `createdByUserId` and `Appointment.createdAt`. Appointment status and queue status repositories pass their guarded `didTransition`/`didAppointmentTransition` result to the shared activity recorder. False means no activity. The arrival helper result is captured; only `wasEstablished` produces `PATIENT_ARRIVED`, with its persisted outcome. Arrival-plus-queue/called skips share the operation timestamp and are returned arrival-first.
