...
Historical planning note: this document records the v0.2 transition plan and older deployment assumptions. Use [Product Requirements](../PRODUCT_REQUIREMENTS.md) for current capability status and [High-Level Design](../HIGH_LEVEL_DESIGN.md) for current architecture. Live deployment URLs, deployed commit SHAs, and smoke-test results remain owner verification required unless recorded with evidence.
...
Do not overwrite `docs/product/MVP.md` as though the MVP never existed.

Update it with a release status notice near the top:

```txt
Status: Completed and frozen as Pravaah v0.1.0.
Active post-MVP scope: docs/scope/V0_2_SCOPE.md.
```
...
Update `docs/scope/ROADMAP.md` to:
...
Update `docs/architecture/ARCHITECTURE.md` with:
...
Update `docs/product/USER_ROLES.md` to clarify:
...
Update `docs/architecture/DATABASE_DESIGN.md` only with confirmed requirements:
...
Update `docs/guides/CONTRIBUTING.md`:
...
Update `docs/guides/SETUP.md` later in the implementation cycle with actual Clerk sign-up URLs and onboarding commands; in the scope PR, add only confirmed high-level guidance.
...
- `docs/scope/V0_2_SCOPE.md` exists
